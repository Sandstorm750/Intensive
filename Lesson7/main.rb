...
class Main
  ...
  def create_wagon
    begin
      ...
      if type == 1
        ...
        unless volume > 138 || volume < 114
          @wagon_list << CargoWagon.new(number, :cargo, volume)
        else
          return puts "Объём Товарного вагона не может быть более 138 и менее 114 куб.метров."
        end
        ...
        unless number_seats > 58 || number_seats <= 0
          @wagon_list << PassengerWagon.new(number, :passenger, number_seats)
        else
          return puts "Число мест в Пассажирском вагоне не может быть более 58 или менее 1."
        end
      else
  ...
  def train_list
    ...
    if st > @station_list.length || st <= 0
      ...
      @station_list[st - 1].train_list.length <= 0
  ...
  def wagons_in_train
    ...
    if @trains[num - 1].is_a?(PassengerTrain)
      @trains[num - 1].wagon_catcher do |wagon|
        puts " - Пассажирский вагон № #{wagon.number} имеет свободных мест: #{wagon.free_seats}, и занятых:#{wagon.occupied_seats}"
      end
    else 
      @trains[num - 1].is_a?(CargoTrain)
      @trains[num - 1].wagon_catcher do |wagon|
        puts " - Вагон № #{wagon.number} типа #{wagon.type} имеет свободный объём: #{wagon.free_volume} кв.метров, и занятый:#{wagon.occupied_volume} кв.метров"
      end      
    end    
  end
